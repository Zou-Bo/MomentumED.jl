...
@testset "MBOperator * MBS64Vector" begin
    ...
    space = HilbertSubspace(list)
    
    # Input |01>
    vec_in_val = zeros(ComplexF64, length(space))
    vec_in_val[get(space, MBS64(2, (1,)))] = 1.0
    vec_in = MBS64Vector(vec_in_val, space)
    vec_out = op * vec_in
    @test vec_out.vec[get(space, MBS64(2, (2,)))] ≈ 1.0

    # Input |10>
    vec_in_val = zeros(ComplexF64, length(space))
    vec_in_val[get(space, MBS64(2, (2,)))] = 1.0
    vec_in = MBS64Vector(vec_in_val, space)
    vec_out = op * vec_in
    @test vec_out.vec[get(space, MBS64(2, (1,)))] ≈ 1.0
end
...
@testset "MBOperator * MBS64Vector (upper_hermitian)" begin
    ...
    vec_in = MBS64Vector(vec_in_val, space)
    vec_out = op * vec_in
    @test vec_out.vec[get(space, MBS64(2, (1,)))] ≈ 0.0
    @test vec_out.vec[get(space, MBS64(2, (2,)))] ≈ 1.0 - 1.0im
    ...
    vec_in = MBS64Vector(vec_in_val, space)
    vec_out = op * vec_in
    @test vec_out.vec[get(space, MBS64(2, (1,)))] ≈ 1.0 + 1.0im
    @test vec_out.vec[get(space, MBS64(2, (2,)))] ≈ 0.0
end
