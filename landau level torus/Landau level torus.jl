...
module LLT
    ...
    export orbital_overlap, density_operator
    ...
    function density_operator(q1::Int64, q2::Int64, cf::Int64=1, ci::Int64=1;
        para::EDPara, level_Lauguerre::Bool)::MBOperator
        cp = para.H_two.components
        Gk = para.Gk
        k_list = para.k_list
        bits = para.Nc * para.Nk

        @assert para.H_two isa LandauInteraction "the ED parameter is not generated by LandauInteraction in LLT module."
        ...
        ql = (q_coord[1] * para.H_two.lattice.G1 + q_coord[2] * para.H_two.lattice.G2) * l
        C = cp.Chern[ci]
        F = complex(1.0)
        if level_Lauguerre
        ...
        scats = Scatter{ComplexF64, MBS64{bits}}[]
        for ki in axes(k_list, 2)
            ...
            push!(scats, Scatter(sign*F*phase, index_f, index_i; bits = bits, upper_hermitian = false))
        end
